...
public static class Program
{
    public static void Main(string[] args)
    {
        ...
        TaskInput input    = JsonSerializer.Deserialize<TaskInput>(json, options)!;

        var pertEngine    = new PertEngine();
        PertResult result = pertEngine.Estimate(input);

        string riskDisplay = result.RiskLevel switch
        {
            RiskLevel.Low    => "Baixo",
        ...
        Console.WriteLine($"  Risco:             {riskDisplay}\n");
    }
}
